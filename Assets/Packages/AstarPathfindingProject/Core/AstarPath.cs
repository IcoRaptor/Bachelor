...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    private bool graphUpdatesWorkItemAdded = false;
    ...
    public static void FindAstarPath()
    {
        if (Application.isPlaying)
            return;
        if (active == null)
            active = GameObject.FindObjectOfType<AstarPath>();
    ...
    private void RecalculateDebugLimits()
    {
        ...
        if (debugRoof - debugFloor < 1)
            debugRoof += 1;
    }

    private Pathfinding.Util.RetainedGizmos gizmos = new Pathfinding.Util.RetainedGizmos();
